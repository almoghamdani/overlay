...
bool overlay::helper::injection::inject_dll_to_thread(std::string dll, DWORD tid) {
    ...
    if (!lib) {
        ...
        std::cerr << "[OverlayHelper] Unable to get proc address of msg hook function!"
                  << std::endl;
        return false;
    }
    ...
    // Post message for the dest thread to trigger loading the library in the dest process
    for (unsigned int i = 0; i < MSG_POST_TIMES; i++) {
        if (!PostThreadMessage(tid, WM_USER + 432, 0, 0)) {
            std::cerr << "[OverlayHelper] Unable to post message to the dest thread!" << std::endl;
            return false;
        }
...
bool OverlayInjectToProcess(DWORD pid) {
    DWORD tid = overlay::helper::utils::get_main_thread_id_for_process(pid);
    ...
    if (!overlay::helper::injection::inject_dll_to_thread(CORE_DLL_NAME, tid)) {
